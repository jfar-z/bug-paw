...
describe("会话文本系统工具目录", () => {
  it("为新旧 Agent 提供低风险的搜索与阅读权限", () => {
    ...
    expect(DEFAULT_AGENT_TOOL_NAMES).toEqual(expect.arrayContaining(["session_search", "session_read"]));
    expect(SYSTEM_TOOL_NAMES).toEqual(expect.arrayContaining(["session_search", "session_read"]));
  });
});
