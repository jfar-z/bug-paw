...
describe("MediaAttachment", () => {
  ...
  it("按媒体类型展示图片、音频和视频", () => {
    const { rerender } = render(<MediaAttachment file={workspaceFile("image/png", "图片.png")} />);
  ...
  it("图片提供全屏预览入口，视频只保留原生控制条", () => {
    ...
    const { rerender } = render(<MediaAttachment file={workspaceFile("image/png", "图片.png")} onPreview={onPreview} />);
  ...
  it("普通文件展示大小且所有类型都有快捷下载", () => {
    render(<MediaAttachment file={workspaceFile("application/pdf", "报告.pdf")} />);
  ...
  it("仅有相对路径时通过 HEAD 加载文件元数据", async () => {
    ...
    render(<MediaAttachment file={{ path: "attachments/agent-output.png" }} />);
