...
import type { BrowserAutomationConfig, BrowserAutomationSettingsDocument, TrustedBrowserOrigin } from "../../shared/browser-automation-contracts";
...
export function BrowserAutomationPage() {
  ...
  return <main className="configuration-page browser-automation-page">
    ...
    {offline ? <p className="browser-status-banner" role="status">离线只读 · 显示上次保存的本地快照，测试与保存已暂停。</p> : null}
    ...
    <SettingsSection index={1} title="服务状态" description="部署与资源摘要不包含 Context 或 Agent 身份">
      ...
      <div className="browser-runtime-strip"><span data-state={document.deployment.workerAvailable ? "ok" : "off"}>{document.deployment.workerAvailable ? "Worker 可用" : "Worker 不可用"}</span><span>{document.deployment.activeContexts} 个活动 Context</span><span>{document.deployment.queuedRequests} 个排队任务</span></div>
    ...
    <SettingsSection index={2} title="公开浏览范围" description="第一期固定 HTTPS；空域名清单表示允许全部公网站点">
      <p className="browser-policy-value"><strong>所有公网 HTTPS 站点</strong><small>私网、回环、链路本地、云元数据和重绑定地址始终由受控出口拒绝。</small></p>
      <div className="browser-number-grid"><NumberField label="导航超时（秒）" value={draft.publicBrowsing.navigationTimeoutMs / 1000} min={10} max={120} onChange={(value) => patch({ publicBrowsing: { ...draft.publicBrowsing, navigationTimeoutMs: value * 1000 } })} /><NumberField label="单 Run 打开上限" value={draft.publicBrowsing.maxPagesPerRun} min={1} max={100} onChange={(value) => patch({ publicBrowsing: { ...draft.publicBrowsing, maxPagesPerRun: value } })} /></div>
    </SettingsSection>

    <SettingsSection index={3} title="受信任 UI 验证" description="交互权限只对精确 Scheme、Host 和 Port 生效">
      <div className="browser-origin-add"><label><span>精确 Origin</span><input aria-label="新增受信任 Origin" value={origin} placeholder="https://ui.example.com" onChange={(event) => setOrigin(event.target.value)} /></label><button type="button" className="configuration-secondary-action" onClick={addOrigin}><Plus size={15} aria-hidden="true" />添加 Origin</button></div>
      {draft.trustedOrigins.length === 0 ? <p className="configuration-help">尚未信任任何 UI Origin；公开网页只能只读游览。</p> : <div className="browser-origin-list">{draft.trustedOrigins.map((item, index) => <OriginCard key={item.origin} value={item} onChange={(next) => patch({ trustedOrigins: draft.trustedOrigins.map((candidate, candidateIndex) => candidateIndex === index ? next : candidate) })} onRemove={() => patch({ trustedOrigins: draft.trustedOrigins.filter((_, candidateIndex) => candidateIndex !== index) })} />)}</div>}
    </SettingsSection>

    <SettingsSection index={4} title="本地静态页面" description="通过临时内部 Origin 预览当前 Agent 工作区 HTML">
    ...
    <SettingsSection index={5} title="资源池" description="默认值为本地慢模型留出充足思考时间">
    ...
    <SettingsSection index={6} title="浏览产物" description="截图和下载只写入当前 Agent 工作区的系统目录">
...
function OriginCard({ value, onChange, onRemove }: { value: TrustedBrowserOrigin; onChange: (value: TrustedBrowserOrigin) => void; onRemove: () => void }) {
  return <article className="browser-origin-card"><header><strong>{value.origin}</strong><button type="button" aria-label={`删除 ${value.origin}`} onClick={onRemove}><Trash2 size={15} aria-hidden="true" /></button></header><PermissionSwitches value={value} onChange={(next) => onChange({ ...value, ...next })} /></article>;
}
...
function PermissionSwitches({ value, onChange }: { value: Omit<TrustedBrowserOrigin, "origin">; onChange: (value: Omit<TrustedBrowserOrigin, "origin">) => void }) {
  return <div className="browser-permission-grid">{([{ key: "allowTextInput", label: "允许文本输入" }, { key: "allowFormSubmit", label: "允许表单提交" }, { key: "allowFileUpload", label: "允许文件上传" }] as const).map(({ key, label }) => <label key={key}><input aria-label={label} type="checkbox" checked={value[key]} onChange={(event) => onChange({ ...value, [key]: event.target.checked })} /><span>{label}</span></label>)}</div>;
}
