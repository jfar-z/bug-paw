...
- `knowledge_search` 可用时，注入内部、项目或用户资料应优先检索知识库的触发条件；`knowledge_read` 同时可用时，再说明重要结论不能只依赖不完整摘录。
- `web_search` 可用时，注入显式查找、时效性、精确归因和不确定事实应联网核验的触发条件；`web_read` 同时可用时，再说明重要事实应读取相关来源正文。
