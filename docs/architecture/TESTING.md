...
当前 gzip 预算为 JavaScript 入口 230 KiB、单个懒加载块 150 KiB、CSS 首屏 46 KiB、CSS 全站总量 56 KiB。页面 CSS 按相对首屏新增的静态依赖单独计费，首批覆盖 AIGC 工作台与 AIGC 渠道页，各 2.5 KiB。Bundle 检查还会拒绝 Manifest 中的 Chunk 导入环，防止循环初始化导致生产白屏。新增页面预算必须先完成样式分包并根据生产构建基线设置，不得使用空页面或全局样式掩盖真实增量。
